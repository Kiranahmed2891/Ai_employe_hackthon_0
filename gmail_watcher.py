...
VAULT_PATH = Path.home() / "Desktop" / "Hack00"
...
def setup_directories():
    ...
    directories = [
        tasks_path,  # 01_Incoming_Tasks
        Path(VAULT_PATH) / "02_In_Progress_Tasks",
        Path(VAULT_PATH) / "03_Completed_Tasks",
        Path(VAULT_PATH) / "04_Archived_Emails",
        Path(VAULT_PATH) / "Logs"
    ]
    ...
    for directory in directories:
        try:
            if not directory.exists():
                ...
                print(f"📁 [{datetime.now().strftime('%H:%M:%S')}] Directory exists: {directory}")
        except PermissionError:
            print(f"❌ [{datetime.now().strftime('%H:%M:%S')}] Permission denied creating directory: {directory}")
            print(f"💡 [{datetime.now().strftime('%H:%M:%S')}] Please run as administrator or choose a different location")
            raise
        except Exception as e:
            print(f"❌ [{datetime.now().strftime('%H:%M:%S')}] Error creating directory {directory}: {str(e)}")
            raise
